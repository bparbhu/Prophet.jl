...
function neural_prophet_mean_from_A(
    k, m, delta, beta, t, cap, A, t_change, X, trend_indicator, s_a, s_m,
    X_seasonality, X_autoregression, W_seasonality, W_autoregression,
)
    base_mu = prophet_mean_from_A(k, m, delta, beta, t, cap, A, t_change, X, trend_indicator, s_a, s_m)
    nn = NeuralProphetNN(W_seasonality, W_autoregression)
    return base_mu .+ nn(X_seasonality, X_autoregression)
end
...
@model function neural_prophet(
    t, X, y, A, t_change, s_a, s_m, tau, sigmas, X_seasonality, X_autoregression;
    cap=zeros(length(t)),
    trend_indicator=0,
)
    T = length(t)
